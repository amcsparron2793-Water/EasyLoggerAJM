...
class TestEasyLogger:
    def test_creation(self, easy_logger_default):
        assert isinstance(easy_logger_default, EasyLogger)

    def test_logger_inst_creation(self, default_logger):
        assert isinstance(default_logger, _EasyLoggerCustomLogger)

    def test_logger_non_custom_logger_inst_creation(self, test_attrs):
    ...
    def test_project_name(self, easy_logger_default):
        assert easy_logger_default.project_name == "TestProject"

    def test_default_format(self, easy_logger_default):
        assert easy_logger_default.DEFAULT_FORMAT == '%(asctime)s | %(name)s | %(levelname)s | %(message)s'

    def test_inner_log_fstructure(self, easy_logger_default):
        assert easy_logger_default.inner_log_fstructure is not None

    def test_log_location(self, easy_logger_default, test_attrs):
    ...
    def test_use_logger_creation(self, test_attrs):
        # UseLogger should return the logger instance
        logger = EasyLogger.UseLogger(project_name="TestProject2",
                                      root_log_location=f"{test_attrs['root_log_location']}2")
        assert isinstance(logger, _EasyLoggerCustomLogger)

    def test_make_file_handlers(self, easy_logger_default, default_logger):
    ...
    def test_logger_level_normalization_with_kwargs(self, test_attrs):
    ...
    def test_is_daily_log_spec(self, test_attrs):
