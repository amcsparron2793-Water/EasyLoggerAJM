...
class TestStreamHandlerIgnoreExecInfo:
    def test_emit_without_exc_info(self, handler, stream):
        record = logging.LogRecord("my_logger", logging.INFO, "dummy_path", 0, "Hello, world!", None, None)
        handler.emit(record)
        assert stream.getvalue() == "Hello, world!\n"

    def test_emit_with_exc_info(self, handler, stream):
        try:
    ...
    def test_emit_restores_exc_info(self, handler, stream):
        try:
